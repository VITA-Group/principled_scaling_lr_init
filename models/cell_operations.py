...
__all__ = ['NAS_BENCH_201', 'OPS', 'ResNetBasicblock', 'SearchSpaceNames']
...
class ResNetBasicblock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride, affine=True):
        ...
        _init(self.conv_a, degree=2, mode=MODE)
        # _init(self.conv_a, degree=1, mode=MODE) # baseline
        self.conv_b = ReLUConvBN(planes, planes, 3,      1, 1, 1, affine)
        _init(self.conv_b, degree=1, mode=MODE)
        if stride == 2:
        ...
        if self.downsample: _init(self.downsample, degree=2, mode=MODE)
        # if self.downsample: _init(self.downsample, degree=1, mode=MODE) # baseline
